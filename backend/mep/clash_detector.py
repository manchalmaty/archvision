...
from models import RoomLayout, MEPConflict
                ...
                for k in range(len(pipe_a.points) - 1):
                    for m in range(len(pipe_b.points) - 1):
                        dist = segment_min_distance(
                            pipe_a.points[k], pipe_a.points[k + 1],
                            pipe_b.points[m], pipe_b.points[m + 1],
                        )
                        min_clearance = (pipe_a.diameter_mm + pipe_b.diameter_mm) / 2000 + CLASH_RADIUS
        ...
        for pipe in self.pipes:
            for room in self.rooms:
                ...
                for k in range(len(pipe.points) - 1):
                    ...
                    if (room.x - 0.1 <= px <= room.x + room.width + 0.1 and
                            room.y - 0.1 <= py <= room.y + room.depth + 0.1 and
                            z_floor <= pz <= z_ceil):
                        # Check if pipe is ON a wall (not interior)
                        on_wall = (
                            abs(px - room.x) < 0.1 or
                            abs(px - (room.x + room.width)) < 0.1 or
                            abs(py - room.y) < 0.1 or
                            abs(py - (room.y + room.depth)) < 0.1
                        )
                        if on_wall:
                            conflicts.append(MEPConflict(
                                conflict_id=str(uuid.uuid4()),
